...
fn file_name_check(file_name: &str) -> &'static str {
    ...
    if parts.len() == 2
        ...
        && parts[0].matches(char::is_numeric).count() <= 3
...
fn main() {
    ...
    println!("{}", file_name_check("2file.txt")); // Should print "No"
    println!("{}", file_name_check("file1234.txt")); // Should print "No"
    println!("{}", file_name_check("file.exe")); // Should print "Yes"
    println!("{}", file_name_check("file.txt.zip")); // Should print "No"
}

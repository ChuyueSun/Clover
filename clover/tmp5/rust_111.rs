...
fn histogram(test: &str) -> HashMap<char, usize> {
    ...
    for ch in test.chars().filter(|c| c.is_alphabetic()) {
        let count = freq_map.entry(ch).or_insert(0);
        *count += 1;
        max_count = max_count.max(*count);
    }

    // Collect letters that have the maximum occurrence
    freq_map.into_iter()
...
fn main() {
    let test_string = "a b c d a a b b b c";
    let result = histogram(test_string);
    println!("{:?}", result);
}
`
#[cfg(test)]

...
fn search(lst: Vec<i32>) -> i32 {
    ...
    // Find the greatest number with the required frequency condition
    freqs.iter()
        .filter(|(&&num, &count)| num <= count && num > 0)
        .map(|(&num, _)| num)
        .max()
        .unwrap_or(-1)
}
...
fn main() {
    // Example usage:
    let nums = vec![3, 8, 2, 3, 3, 2];
    println!("The output is: {}", search(nums));
}

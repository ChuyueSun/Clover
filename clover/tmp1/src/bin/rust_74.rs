...
fn total_match(lst1: Vec<&str>, lst2: Vec<&str>) -> Vec<&str> {
    let count_chars_lst1 = lst1.iter().fold(0, |acc, &s| acc + s.chars().count());
    let count_chars_lst2 = lst2.iter().fold(0, |acc, &s| acc + s.chars().count());

...
fn search(lst: Vec<i32>) -> i32 {
    ...
    // Iterate through the frequencies to find the largest number
    // that satisfies the condition (frequency >= number itself)
    let mut result = -1;
    for (&number, &count) in frequencies.iter() {
        if number > 0 && count >= number && number > result {
            result = number;
        }
    }
...
fn main() {
    // Example usage:
    let lst = vec![3, 8, 2, 3, 3, 2];
    println!("Result: {}", search(lst)); // Should output 3
}


use std::collections::HashMap;

fn histogram(text: &str) -> HashMap<char, usize> {
    let mut frequency_map = HashMap::new();
    let mut max_count = 0;

    // Count the frequency of each character in the string.
    for ch in text.chars().filter(|c| c.is_alphabetic()) {
        let counter = frequency_map.entry(ch).or_insert(0);
        *counter += 1;
        max_count = max_count.max(*counter);
    }

    // Identify the characters with the maximum frequency.
    frequency_map
        .into_iter()
        .filter(|&(_, v)| v == max_count)
        .collect()
}

fn main() {
    // Example usage:
    let result = histogram("a b a c d e f f f");
    println!("{:?}", result); // Outputs something like: {'f': 3, 'a': 2} depending on the actual string
}

#[cfg(test)]
mod tests {
    use super::*;

    #[test]
    fn test_histogram() {
        assert!(histogram("a b b a") == HashMap::from([('a', 2), ('b', 2)]));
        assert!(histogram("a b c a b") == HashMap::from([('a', 2), ('b', 2)]));
        assert!(
            histogram("a b c d g")
                == HashMap::from([('a', 1), ('b', 1), ('c', 1), ('d', 1), ('g', 1)])
        );
        assert!(histogram("r t g") == HashMap::from([('r', 1), ('t', 1), ('g', 1)]));
        assert!(histogram("b b b b a") == HashMap::from([('b', 4)]));
        assert!(histogram("r t g") == HashMap::from([('r', 1), ('t', 1), ('g', 1)]));
        assert!(histogram("") == HashMap::new());
        assert!(histogram("a") == HashMap::from([(('a', 1))]));
    }

}

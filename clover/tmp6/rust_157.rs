...
fn right_angle_triangle(a: f32, b: f32, c: f32) -> bool {
    const EPSILON: f32 = 0.001; // To account for floating point precision issues
    let mut sides = [a, b, c];
    sides.sort_by(|a, b| a.partial_cmp(b).unwrap());

    // Pythagorean theorem: a^2 + b^2 = c^2
    // The longest side should be the hypotenuse
    let (side1, side2, hypotenuse) = (sides[0], sides[1], sides[2]);

    ((side1 * side1 + side2 * side2 - hypotenuse * hypotenuse).abs() < EPSILON)
}

fn main() {
    // Example use
    let result = right_angle_triangle(3.0, 4.0, 5.0);
    println!("Is triangle a right-angled triangle? {}", result);
}

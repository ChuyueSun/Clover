...
fn find_max(words: Vec<&str>) -> &str {
    words
        .iter()
        .max_by(|a, b| {
            let uniq_a = a.chars().collect::<std::collections::HashSet<_>>().len();
            let uniq_b = b.chars().collect::<std::collections::HashSet<_>>().len();
            uniq_a.cmp(&uniq_b).then_with(|| a.cmp(&b).reverse())
        })
        .unwrap_or(&"")
}

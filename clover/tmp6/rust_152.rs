...
fn compare(game: Vec<i32>, guess: Vec<i32>) -> Vec<i32> {
    game.iter().zip(guess.iter())
        .map(|(&result, &prediction)| (result - prediction).abs())
        .collect()
}

fn main() {
    // Example usage:
    let game_scores = vec![10, 15, 13];
    let guesses = vec![10, 14, 12];
    let differences = compare(game_scores, guesses);
    println!("{:?}", differences); // Should print: [0, 1, 1]
}

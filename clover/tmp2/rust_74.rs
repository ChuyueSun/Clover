...
fn total_match(lst1: Vec<&str>, lst2: Vec<&str>) -> Vec<&str> {
    let count_chars = |v: &Vec<&str>| v.iter().fold(0, |acc, &s| acc + s.len());

    let chars_count1 = count_chars(&lst1);
    let chars_count2 = count_chars(&lst2);

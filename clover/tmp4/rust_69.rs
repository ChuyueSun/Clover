...
fn search(lst: Vec<i32>) -> i32 {
    ...
    // Initialize the result with -1, assuming no such number exists
    let mut result = -1;
    
    // Iterate through the frequency map to search for a valid number
...
fn main() {
    // Test cases
    println!("{}", search(vec![1, 2, 2, 3, 3, 3])); // Should print 3
    println!("{}", search(vec![1])); // Should print -1
    println!("{}", search(vec![5, 5, 5, 5, 5])); // Should print 5
}

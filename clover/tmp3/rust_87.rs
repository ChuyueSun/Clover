...
fn get_row(lst: Vec<Vec<i32>>, x: i32) -> Vec<(usize, usize)> {
    ...
    for (i, row) in lst.iter().enumerate() {
        for (j, &item) in row.iter().enumerate() {
            if item == x {
                coordinates.push((i, row.len() - 1 - j));
            }
        }
    }
    ...
    // Convert columns back to correct order after sorting
    for c in &mut coordinates {
        c.1 = lst[c.0].len() - 1 - c.1;
    }
...
fn main() {
    ...
    let x = 1;
    let result = get_row(nested_list, x);
    println!("{:?}", result);
    // Should print: [(0, 0), (1, 3), (1, 2), (2, 0)]
}

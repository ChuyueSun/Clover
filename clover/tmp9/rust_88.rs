
fn sort_array(mut array: Vec<i32>) -> Vec<i32> {
    array.sort_unstable_by(|a, b| {
        let ones_a = a.count_ones();
        let ones_b = b.count_ones();
        ones_a.cmp(&ones_b).then_with(|| a.cmp(b))
    });
    array
}

#[cfg(test)]
mod tests {
    use super::*;

    #[test]
    fn it_works_for_empty_array() {
        let array = vec![];
        assert_eq!(sort_array(array), []);
    }

    #[test]
    fn it_works_for_single_element() {
        let array = vec![2];
        assert_eq!(sort_array(array), [2]);
    }

    #[test]
    fn it_sorts_correctly() {
        let array = vec![0, 1, 2, 3, 4, 5, 6, 7, 8];
        assert_eq!(sort_array(array), [0, 1, 2, 4, 8, 3, 5, 6, 7]);
    }

    #[test]
    fn it_sorts_correctly_with_equal_number_of_ones() {
        let array = vec![3, 1, 2];
        assert_eq!(sort_array(array), [1, 2, 3]);
    }
}

fn main() {
    println!("{:?}", sort_array(vec![0, 1, 2, 3, 4, 5, 6, 7, 8]));
}
`
#[cfg(test)]
mod tests {
    use super::*;

  #[test]
    fn test_sort_array() {
        assert!(sort_array(vec![]) == vec![]);
        assert!(sort_array(vec![5]) == vec![5]);
        assert!(sort_array(vec![2, 4, 3, 0, 1, 5]) == vec![0, 1, 2, 3, 4, 5]);
        assert!(sort_array(vec![2, 4, 3, 0, 1, 5, 6]) == vec![6, 5, 4, 3, 2, 1, 0]);
        assert!(sort_array(vec![2, 1]) == vec![1, 2]);
        assert!(sort_array(vec![15, 42, 87, 32, 11, 0]) == vec![0, 11, 15, 32, 42, 87]);
        assert!(sort_array(vec![21, 14, 23, 11]) == vec![23, 21, 14, 11]);
    }

}

...
fn rescale_to_unit(numbers: Vec<f32>) -> Vec<f32> {
    if let (Some(&min), Some(&max)) = (numbers.iter().min_by(|a, b| a.partial_cmp(b).unwrap()), 
                                        numbers.iter().max_by(|a, b| a.partial_cmp(b).unwrap())) {
        if (max - min).abs() > f32::EPSILON {
            numbers.iter().map(|&x| (x - min) / (max - min)).collect()
        } else {
            vec![0.0; numbers.len()] // If all elements are the same.
        }
    } else {
        vec![] // If the input vector is empty.
    }
}

fn main() {
    // Example usage:
    let numbers = vec![2.0, 5.0, 1.0, 9.0];
